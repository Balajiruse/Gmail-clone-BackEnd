...
app.put("/gmail_data/:id", async function (request, response) {
  ...
  ?response.send({msg:"Gmail successfully updated"})
  :response.status(400).send({msg:"gmail not found"});
});
...
app.delete("/gmail_data/:name", async function (request, response) {
  ...
  ?response.send({msg:"gmail successfully deleted"})
  :response.status(401).send({msg:"gmail not found"});
});
